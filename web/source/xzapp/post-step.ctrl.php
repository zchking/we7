...
if ($step == 2) {
	...
	if (checksubmit('submit')) {
		...
		$update['type'] = ACCOUNT_TYPE_XZAPP_NORMAL;
		...
		if (!empty($_GPC['uniacid']) || empty($_W['isfounder'])) {
			header("Location: ".url('xzapp/post-step/', array('uniacid' => $uniacid, 'acid' => $acid, 'step' => 4)));
		} else {
			header("Location: ".url('xzapp/post-step/', array('uniacid' => $uniacid, 'acid' => $acid, 'step' => 3)));
		}
		exit;
	}
}
...
if ($step == 3) {
	...
	if (checksubmit('submit')) {
		...
		if (!empty($_GPC['from'])) {
			itoast('熊掌号权限修改成功', url('xzapp/post-step/', array('uniacid' => $uniacid, 'step' => 3, 'from' => 'list')), 'success');
		} else {
			header("Location: ".url('xzapp/post-step/', array('uniacid' => $uniacid, 'acid' => $acid, 'step' => 4)));
			exit;
		}
